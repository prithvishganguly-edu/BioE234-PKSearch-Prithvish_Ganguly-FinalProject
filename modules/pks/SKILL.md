...
2. Call `assess_pks_feasibility` to pre-screen the target
3. If feasible (score ≥ 0.6):
   - Call `pks_design_retrotide` with the SMILES
   - Present the full results including all module domains
4. If not feasible (score < 0.6):
   - Inform the user which checks failed and why
   - Call `tridentsynth` for PKS + tailoring pathway design
